...
- the vendored Node OpenAPI package at `node/vendor/zod-to-openapi`, cloned from
  the intended `zod-to-openapi` repository before the first image build;
...
The Node OpenAPI package is also intentionally kept outside Git because it may
come from a private repository. Clone or copy it into the checkout so that
`node/vendor/zod-to-openapi/package.json` exists before building the Node
images. The Docker build compiles that package inside the image and does not
need an SSH agent or network access to the private repository. Keep the vendor
directory at that exact path; a clone elsewhere is outside the Docker build
context.

Example:

```powershell
git clone <zod-to-openapi-repository-url> node/vendor/zod-to-openapi
Test-Path node/vendor/zod-to-openapi/package.json
```

If the repository is already cloned elsewhere, copy it to that path instead.
Do not commit the vendor directory unless the dependency is intentionally being
made public.
