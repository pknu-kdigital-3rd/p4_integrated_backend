...
- the vendored Node OpenAPI package at `node/vendor/zod-to-openapi`;
...
The Node OpenAPI package is checked into Git at that path, so a normal checkout
already contains `package.json` and the source needed for the Docker build. The
Docker build compiles it locally and does not need an SSH agent or access to a
private Git repository. If the vendored package is updated, replace the tracked
source from the intended upstream or fork, omit its nested `.git` directory,
and review the resulting dependency changes before committing.
