...
For a host with multiple CUDA devices, this branch can run stateless
segmentation concurrently. Set this in `deploy/env.local` before starting
Vision:
...
Use `export YOLO_PARALLEL_DEVICES="auto"` to select every CUDA device visible
to PyTorch.

Vision loads one model copy per listed device and reorders completed results by
source sequence before publishing them. This branch does not assign
`track_id` values. Leave the value empty for the normal single-GPU path. This
mode uses additional VRAM for each model copy and only helps when both devices
are available and segmentation is the bottleneck.
