...
function renderReplayBreaks(){const breaks=document.querySelector('#recording-breaks'),markers=document.querySelector('#recording-break-markers');breaks.replaceChildren();markers.replaceChildren();for(const entry of replayTimeline){if(entry.breakBefore){const note=document.createElement('span');const gap=Math.max(0,entry.wallGapSeconds);note.textContent=`Break before segment ${entry.video.segmentIndex}: ${formatReplayGap(gap)} unrecorded`;breaks.append(note,document.createElement('br'));const marker=document.createElement('span');marker.style.left=`${replayDuration?entry.start/replayDuration*100:0}%`;marker.title=`Recording break before segment ${entry.video.segmentIndex}`;markers.append(marker)}}}
