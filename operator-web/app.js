...
function closeRecordingDeleteMode(){document.querySelector('#recording-delete-mode').hidden=true;document.querySelector('#recording-delete-toggle').textContent='Delete segments';document.querySelector('#recording-delete-list').replaceChildren();document.querySelector('#recording-delete-selected').disabled=true}
function renderRecordingDeleteList(){const list=document.querySelector('#recording-delete-list');list.replaceChildren();for(const entry of replayTimeline){const row=document.createElement('li'),checkbox=document.createElement('input'),label=document.createElement('label'),video=entry.video;checkbox.type='checkbox';checkbox.value=video.tripVideoId;checkbox.id=`delete-trip-video-${video.tripVideoId}`;checkbox.addEventListener('change',()=>{document.querySelector('#recording-delete-selected').disabled=!list.querySelector('input:checked')});label.htmlFor=checkbox.id;label.textContent=`Segment ${video.segmentIndex} · ${video.startedAt?new Date(video.startedAt).toLocaleString():'time unavailable'}`;row.append(checkbox,label);list.append(row)}}
async function deleteSelectedRecordingSegments(){const list=document.querySelector('#recording-delete-list'),selected=[...list.querySelectorAll('input:checked')].map(input=>input.value);if(!selected.length)return;const prompt=`Permanently delete ${selected.length} recording segment${selected.length===1?'':'s'} from trip ${replayTripId} and their replay detections?`;if(!window.confirm(prompt))return;const button=document.querySelector('#recording-delete-selected'),tripId=replayTripId;button.disabled=true;stopRecordingPlayback();document.querySelector('#recordings-status').textContent=`Deleting ${selected.length} recording segment${selected.length===1?'':'s'}…`;let deleted=0;const failures=[];for(const tripVideoId of selected){try{await api(`/api/v1/trip-videos/${encodeURIComponent(tripVideoId)}`,{method:'DELETE'},true);deleted++}catch(ex){failures.push(ex.message)}}await loadTripRecordings(tripId);document.querySelector('#recordings-status').textContent=failures.length?`Deleted ${deleted} of ${selected.length} segments. ${failures.length} failed: ${failures[0]}`:`Deleted ${deleted} segment${deleted===1?'':'s'} and their replay detections.`}
...
async function loadTripRecordings(value){closeRecordingDeleteMode();document.querySelector('#recording-delete-tools').hidden=true;const requestId=++recordingsRequest;const tripId=String(value||'').trim(),status=document.querySelector('#recordings-status');replayTimeline=[];replayDuration=0;replayTripId=tripId;stopRecordingPlayback();document.querySelector('#recording-player-panel').hidden=true;if(!/^[1-9][0-9]{0,18}$/.test(tripId)){status.textContent='Select a vehicle with a trip or enter a positive Trip ID.';return}status.textContent=`Loading trip ${tripId}…`;try{await requireRecordingLogin();const videos=await api(`/api/v1/trips/${encodeURIComponent(tripId)}/videos`,{},true);if(requestId!==recordingsRequest)return;if(!videos.length){status.textContent=`No finalized recording segments are available for trip ${tripId}.`;return}const timeline=buildReplayTimeline(videos);replayTimeline=timeline.entries;replayDuration=timeline.duration;updateRecordingDeleteTools();if(!replayTimeline.length){status.textContent=`Trip ${tripId} has no segments with a usable duration.`;return}document.querySelector('#recording-player-panel').hidden=false;const seek=document.querySelector('#recording-seek');seek.max=String(replayDuration);seek.value='0';document.querySelector('#recording-time').textContent=`0:00 / ${formatReplayTime(replayDuration)}`;renderReplayBreakMarkers();status.textContent=`Trip ${tripId}: ${replayTimeline.length} segments, ${formatReplayTime(replayDuration)} total. Loading detections as segments play.`;await seekReplay(0,false)}catch(ex){if(requestId!==recordingsRequest)return;status.textContent=ex.message}}
...
document.querySelector('#recording-delete-toggle').addEventListener('click',event=>{const mode=document.querySelector('#recording-delete-mode');if(mode.hidden){renderRecordingDeleteList();mode.hidden=false;event.currentTarget.textContent='Exit delete mode';document.querySelector('#recordings-status').textContent='Select the recording segments to permanently delete.'}else{closeRecordingDeleteMode();document.querySelector('#recordings-status').textContent='Delete mode closed.'}});
