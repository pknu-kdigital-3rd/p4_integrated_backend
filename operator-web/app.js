...
function closeRecordingDeleteMode(){document.querySelector('#recording-delete-mode').hidden=true;document.querySelector('#recording-delete-toggle').textContent='Delete segments';document.querySelector('#recording-timeline').classList.remove('is-delete-mode');document.querySelector('#recording-seek').disabled=false;document.querySelector('#recording-delete-range').hidden=true;document.querySelector('#recording-delete-selection').hidden=true;document.querySelector('#recording-delete-segments').replaceChildren();document.querySelector('#recording-delete-selected').disabled=true;document.querySelector('#recording-delete-selection-status').textContent='No segments selected.';recordingDeleteRange=null;recordingDeleteDrag=null}
...
async function deleteSelectedRecordingSegments(){if(!recordingDeleteRange)return;const selected=replayTimeline.slice(recordingDeleteRange.first,recordingDeleteRange.last+1),first=selected[0],last=selected.at(-1);if(!selected.length)return;const prompt=`Permanently delete ${selected.length} contiguous recording segment${selected.length===1?'':'s'} from trip ${replayTripId} (segments ${first.video.segmentIndex}–${last.video.segmentIndex}, ${formatReplayTime(first.start)}–${formatReplayTime(last.end)}) and their replay detections?`;if(!window.confirm(prompt))return;const button=document.querySelector('#recording-delete-selected'),tripId=replayTripId;button.disabled=true;stopRecordingPlayback();document.querySelector('#recordings-status').textContent=`Deleting ${selected.length} recording segment${selected.length===1?'':'s'}…`;let deleted=0;const failures=[];for(const entry of selected){try{await api(`/api/v1/trip-videos/${encodeURIComponent(entry.video.tripVideoId)}`,{method:'DELETE'},true);deleted++}catch(ex){failures.push(ex.message)}}await loadTripRecordings(tripId);document.querySelector('#recordings-status').textContent=failures.length?`Deleted ${deleted} of ${selected.length} segments. ${failures.length} failed: ${failures[0]}`:`Deleted ${deleted} segment${deleted===1?'':'s'} and their replay detections.`}
...
document.querySelector('#recording-delete-toggle').addEventListener('click',event=>{const mode=document.querySelector('#recording-delete-mode');if(mode.hidden){renderRecordingDeleteTimeline();mode.hidden=false;document.querySelector('#recording-timeline').classList.add('is-delete-mode');document.querySelector('#recording-seek').disabled=true;document.querySelector('#recording-delete-range').hidden=false;document.querySelector('#recording-delete-range').focus();replayPlayer().pause();event.currentTarget.textContent='Exit delete mode';document.querySelector('#recordings-status').textContent='Drag across the timeline to select a contiguous range of segments.'}else{closeRecordingDeleteMode();document.querySelector('#recording-seek').disabled=false;document.querySelector('#recordings-status').textContent='Delete mode closed.'}});
